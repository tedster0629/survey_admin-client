...
class Launch_survey extends CI_Controller 
{
    ...
    {        
        ...
        $data['check_survey_data'] = $this->Model_launch_survey->listSurveysLaunch();

		$this->load->view('admin/view_header',$data);
		$this->load->view('admin/view_launch_survey', $data);
		$this->load->view('admin/view_footer');
    }

    public function delete_launchSurvey() {

        $id = $this->input->post('id');
        if ($this->input->post('id')) {
            $delete_it = $this->Model_launch_survey->delete_liveSurvey($id);
    ...
    public function view_takeSurvey()
    {
        ...
        if($this->user_completed_survey()){
            $this->load->view('admin/themes/default/view_final');
                ...
                if(isset($id)) {
                    if($this->Model_launch_survey->check_template_survey('theme', 'survey_id', $id)) {
                        $current_theme = $this->Model_launch_survey->check_template_survey('theme', 'survey_id', $id);
                        $theme_path = 'themes/'.$current_theme.'/';
                    }
                }
                if(isset($id) && $this->Model_launch_survey->is_survey_published($id) == true) {
                    $thisID = $this->input->get('id', TRUE);
                    $thisID = intval($thisID); 
                    $sessions = $this->Model_launch_survey->check_template_survey('sessions', 'survey_id', $thisID); 
                    switch($sessions)
                    {
                        case 'Y':
                            // Get Sessions
                            ...
                            if(isset($_GET['page']) && $this->Model_launch_survey->sanitize_integer($_GET['page']) == true)
                            {
                                $page = $_GET['page'];
                            } else {
                                $page = 1;	
                            ...
                            // Pagination Var
                            $per_page = $this->Model_launch_survey->settings_notice(4, 4);
                        ...
                        case 'N':
                            ...
                            // Get results
                            foreach($this->Model_launch_survey->listQuestions($thisID) as $k => $v)
                            {
                                $results[$this->Model_launch_survey->get_question($v->question_ID, 'order')] = $this->Model_launch_survey->get_question($v->question_ID, 'questions');
                            }
                            ksort($results);
                            ...
                            $this->load->view('admin/' . $theme_path . '/view_home_basic', $data);
                            $this->load->view('admin/'. $theme_path . '/view_footer', $data);
                            // redirect(base_url().'contact/view?id=' . $id);
                        break;
    ...
    function get_survey_subHeading()
	{
		...
		$thisID = $this->input->get('id', TRUE);
        $thisID = intval($thisID); 
		$sub_head = $this->Model_launch_survey->check_template_survey('sub_heading', 'survey_id', $thisID);
    ...
    function get_survey_heading()
	{
		...
		$thisID = $this->input->get('id', TRUE);
        $thisID = intval($thisID); 
		$res = $this->Model_launch_survey->check_template_survey('heading', 'survey_id', $thisID);
		if(!empty($res))
		{
    ...
    function get_survey_intro()
	{
		$thisID = $this->input->get('id', TRUE);
        $thisID = intval($thisID); 
		$si = $this->Model_launch_survey->check_template_survey('intro', 'survey_id', $thisID);
		if($si)
		{
    ...
    function get_survey_description()
	{
		$thisID = $this->input->get('id', TRUE);
        $thisID = intval($thisID);
		$se = $this->Model_launch_survey->check_template_survey('description', 'survey_id', $thisID);
		if($se)
		{
    ...
    function user_answered()
	{
		...
		$res = $this->Model_launch_survey->user_already_answered_page($thisID, $this->Model_launch_survey->get_client_ip(), $this->user_took_survey());
    ...
    function user_took_survey()
	{
		$thisID = $this->input->get('id', TRUE);
        $thisID = intval($thisID);
		$res = $this->Model_launch_survey->user_already_took_survey($thisID, $this->Model_launch_survey->get_client_ip()); 
		if($res)
		{
			return $res;
		} else {
    ...
    function get_survey_note()
	{
		$thisID = $this->input->get('id', TRUE);
        $thisID = intval($thisID); 
		return $this->Model_launch_survey->check_template_survey('note', 'survey_id', $thisID);
	}

    function get_question($id, $return)
	{
		
		return $this->Model_launch_survey->get_question($id, $return);	
	}
    ...
    function get_survey_title()
	{
		$thisID = $this->input->get('id', TRUE);
        $thisID = intval($thisID); 
		$title = $this->Model_launch_survey->check_template_survey('title', 'survey_id', $thisID);
    ...
    function get_survey_footer()
	{
		$thisID = $this->input->get('id', TRUE);
        $thisID = intval($thisID);
		return $this->Model_launch_survey->check_template_survey('footer', 'survey_id', $thisID);
	}

    function choices_hint($txt)
	{
		return $this->Model_launch_survey->settings_notice(8, $txt);		
	}
    ...
    public function submitSurvey()
    {
        ...
        $client_ip = $this->Model_launch_survey->get_client_ip();
        ...
        if(isset($_POST['file_upload-(' . $ids.')'])){
            ...
            //validation 
            $fileSize = $_FILES['file_upload-(' . $ids.')']['size'];
            // echo "File size: " . $fileSize;
            $re_fileSize = $this->Model_launch_survey->getfileSize($survey_id, $ids);
            $re_fileSize_string = $re_fileSize[0]["rule"];
            $re_fileSize = intval($re_fileSize_string);
            if($fileSize > $re_fileSize){
                $data['fileSize'] = $re_fileSize_string;
                redirect(base_url() . 'admin/themes/default/val_fileupload?ids=' . $ids . '&survey_id=' . $survey_id);
                // $this->load->view('admin/themes/default/view_val_fileupload', $data);die();           
            }
            //valdation end
            // var_dump($_FILES['file_upload-(' . $ids.')']['name']);die();
            if(PROJECT_MODE == 0) {
            ...
            for ($i = 0; $i <= $total - 1; $i++) {
                ...
                switch ($this->input->post()) {
                    ...
                    case null !== $this->input->post('matrix_single-(' . $q_ids[$i] . ')-' . (($q_ids[$i]) + $z - 1)):
                        // save
                        $this->Model_launch_survey->add_result($survey_id, $q_ids[$i], $this->input->post('matrix_single-(' . $q_ids[$i] . ')-' . (($q_ids[$i]) + $z - 1)), 'matrix_single', $client_ip, $page);
                        break;
                    // Matrix Multi (array)
                    case null !== $this->input->post('matrix_multi_select-(' . $q_ids[$i] . ')'):
                        $array = $this->input->post('matrix_multi_select-(' . $q_ids[$i] . ')');
                        foreach ($array as $v) {
                            $this->Model_launch_survey->add_result($survey_id, $q_ids[$i], $v, 'matrix_multi_select', $client_ip, $page);
                        }
                        break;
                    // Checkbox (array)
                    case null !== $this->input->post('checkbox-(' . $q_ids[$i] . ')'):
                        ...
                        foreach ($array as $v) {
                            $this->Model_launch_survey->add_result($survey_id, $q_ids[$i], $v, 'multiple_choice', $client_ip, $page);
                        }
                        break;
                    // Single
                    case null !== $this->input->post('single-(' . $q_ids[$i] . ')'):
                        $this->Model_launch_survey->add_result($survey_id, $q_ids[$i], $this->input->post('single-(' . $q_ids[$i] . ')'), 'single_text', $client_ip, $page);
                        break;
                    // Radio
                    case null !== $this->input->post('radio-(' . $q_ids[$i] . ')'):
                        $this->Model_launch_survey->add_result($survey_id, $q_ids[$i], $this->input->post('radio-(' . $q_ids[$i] . ')'), 'radio', $client_ip, $page);
                        break;
                    // Comment Box
                    case null !== $this->input->post('suggestion-(' . $q_ids[$i] . ')'):
                        $this->Model_launch_survey->add_result($survey_id, $q_ids[$i], $this->input->post('suggestion-(' . $q_ids[$i] . ')'), 'comment_box', $client_ip, $page);
                        break;
                    // Yes / No
                    case null !==  $this->input->post('yes_no-(' . $q_ids[$i] . ')'):
                        $this->Model_launch_survey->add_result($survey_id, $q_ids[$i], $this->input->post('yes_no-(' . $q_ids[$i] . ')'), 'yes_no', $client_ip, $page);
                        break;
                    // Dropdown Menu
                    case null !== $this->input->post('dropdown_menu-(' . $q_ids[$i] . ')'):
                        $this->Model_launch_survey->add_result($survey_id, $q_ids[$i], $this->input->post('dropdown_menu-(' . $q_ids[$i] . ')'), 'dropdown_menu', $client_ip, $page);
                        break;
                    // Rating
                    case null !== $this->input->post('rating-(' . $q_ids[$i] . ')'):
                        $this->Model_launch_survey->add_result($survey_id, $q_ids[$i], $this->input->post('rating-(' . $q_ids[$i] . ')'), 'rating', $client_ip, $page);
                        break;
                        // file_upload
                    case null !== $this->input->post('file_upload-(' . $q_ids[$i] . ')'):
                        $this->Model_launch_survey->add_result($survey_id, $q_ids[$i], $this->input->post('file_upload-(' . $q_ids[$i] . ')'), 'file_upload', $client_ip, $page);
                        break;
                        // date
                    case null !== $this->input->post('date-(' . $q_ids[$i] . ')'):
                        $this->Model_launch_survey->add_result($survey_id, $q_ids[$i], $this->input->post('date-(' . $q_ids[$i] . ')'), 'date', $client_ip, $page);
                        break;
                        // paragraph
                    case null !== $this->input->post('paragraph-(' . $q_ids[$i] . ')'):
                        $this->Model_launch_survey->add_result($survey_id, $q_ids[$i], $this->input->post('paragraph-(' . $q_ids[$i] . ')'), 'paragraph', $client_ip, $page);
                        break;
                        // short_answer
                    case null !== $this->input->post('short_answer-(' . $q_ids[$i] . ')'):
                        $this->Model_launch_survey->add_result($survey_id, $q_ids[$i], $this->input->post('short_answer-(' . $q_ids[$i] . ')'), 'rating', $client_ip, $page);
                        break;
